...
contract certificate {
    ...
    uint notBefore = block.timestamp;
    uint notAfter=block.timestamp + 365 days;
    ...
    function addUser (string memory _name, string memory _birth, uint _notBefore, uint _notAfter) public{
   ...
   function getCert(uint _idx) public view returns (string memory, string memory, uint, uint){
       return (users[_idx].name, users[_idx].birth, users[_idx].notBefore, users[_idx].notAfter);
   }
